...
export default function AdminPage() {
  ...
  return (
    <div className="p-10 bg-gray-100 min-h-screen">
      <h1 className="text-4xl font-bold mb-10">Manage About Us</h1>
              ...
              <div className="flex justify-center gap-2">
                ...
                <Link
                  ...
                  className="flex items-center gap-1 text-sm px-3 py-1
                            border rounded-md text-blue-600
                ...
                <button
                  onClick={() =>
                    deleteItem(
                      `${API}/api/governing/${m._id}`,
                      () =>
                        fetch(`${API}/api/governing`)
                          .then((r) => r.json())
                          .then(setGoverningBody)
                    )
                  }
                  className="flex items-center gap-1 text-sm px-3 py-1
              ...
              <div className="flex justify-center gap-3">
               ...
               hover:bg-blue-50 hover:border-blue-400 transition"
                ...
                <button
                  onClick={() =>
                    deleteItem(
                      `${API}/api/executive/${m._id}`,
                      () =>
                        fetch(`${API}/api/executive`)
                          .then((r) => r.json())
                          .then(setExecutiveTeam)
                    )
                  }
                  className="flex items-center gap-1 text-sm px-3 py-1
               border border-red-400 text-red-600 rounded-md
          ...
          {certifications.map(c => (
            <tr key={c._id} className="border-t">
              ...
              <td className="p-4 text-center">
                <a href={`${API}${c.pdf}`} target="_blank" className="text-red-600">
                  <FaFilePdf />
                </a>
              </td>
              <td className="p-4">
                <div className="flex justify-center gap-3">
                  <Link href={`/admin/about/certifications/edit/${c._id}`} className="flex items-center gap-1 text-sm px-3 py-1
               border rounded-md text-blue-600
               hover:bg-blue-50 hover:border-blue-400 transition">
                    <FaEdit size={14} /> Edit
                  </Link>
                  <button onClick={() => deleteItem(`${API}/api/certifications/${c._id}`, () =>
                    fetch(`${API}/api/certifications`).then(r => r.json()).then(setCertifications)
                  )} className="flex items-center gap-1 text-sm px-3 py-1
               border border-red-400 text-red-600 rounded-md
               hover:bg-red-50 hover:border-red-500 transition">
        ...
        <tbody>
          {financialPDFs.map(f => (
            <tr key={f._id} className="border-t">
              ...
              <td className="p-4 text-center">
                <a href={`${API}${f.pdf}`} target="_blank" className="text-red-600">
                  <FaFilePdf />
                </a>
              </td>
              <td className="p-4">
                <div className="flex justify-center gap-3">
                  <Link href={`/admin/about/financial/edit/${f._id}`} className="flex items-center gap-1 text-sm px-3 py-1
               border rounded-md text-blue-600
               hover:bg-blue-50 hover:border-blue-400 transition">
                    <FaEdit size={14} /> Edit
                  </Link>
                  <button onClick={() => deleteItem(`${API}/api/financial/${f._id}`, () =>
                    fetch(`${API}/api/financial`).then(r => r.json()).then(setFinancialPDFs)
                  )} className="flex items-center gap-1 text-sm px-3 py-1
               border border-red-400 text-red-600 rounded-md
               hover:bg-red-50 hover:border-red-500 transition">  
